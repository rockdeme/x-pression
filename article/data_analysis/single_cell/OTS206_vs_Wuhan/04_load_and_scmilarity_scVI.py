...
reducer = umap.UMAP(random_state=2207,n_components=2)#change to 2, as we want embedding in 2 dimensions to be explained not 30... 
...
plt.title('umap UMAP of SCVI latent space')
plt.show()
# # %%
# ##########################################################
...
# %%
#######################################
# RUN PHATE and visualize the results #
#######################################
import phate
phate_op = phate.PHATE(n_jobs=-2)
phate_embedding = phate_op.fit_transform(latent_embedding)
adata.obsm["X_phate_X_scVI_latent"] = phate_embedding
...
phate_df = pd.DataFrame({
    ...
    'sample': adata.obs['sample'],
    'phase': adata.obs['phase'],
    "Cd34": adata[:, "Cd34"].layers["log_counts"].toarray().flatten(),
    "Xcr1": adata[:, "Xcr1"].layers["log_counts"].toarray().flatten(),
    "Cd14": adata[:, "Cd14"].layers["log_counts"].toarray().flatten(),
    "pct_counts_scov": adata.obs["pct_counts_scov"],
    "N": adata[:, "N"].layers["log_counts"].toarray().flatten(),
    "S": adata[:, "S"].layers["log_counts"].toarray().flatten(),
    "E": adata[:, "E"].layers["log_counts"].toarray().flatten(),
    "M": adata[:, "M"].layers["log_counts"].toarray().flatten(),
    "Cd8a": adata[:, "Cd8a"].layers["log_counts"].toarray().flatten(),
    "Cd4": adata[:, "Cd4"].layers["log_counts"].toarray().flatten(),
    "Cd19": adata[:, "Cd19"].layers["log_counts"].toarray().flatten(),
    "Nkg7": adata[:, "Nkg7"].layers["log_counts"].toarray().flatten(),
    "Ms4a1": adata[:, "Ms4a1"].layers["log_counts"].toarray().flatten(),

})

# plot
phate_df_shuffled = phate_df.sample(frac=1, random_state=2207).reset_index(drop=True)

fig, axes = plt.subplots(4, 2, figsize=(11, 20))
plt.subplots_adjust(hspace=0.4, wspace=0.4)

sns.scatterplot(data=phate_df_shuffled, x="PHATE1", y="PHATE2", hue="replicates", palette="tab10", s=10, ax=axes[0, 0])
axes[0, 0].set_title("PHATE of Data by Species")
axes[0, 0].legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3, markerscale=2)

sns.scatterplot(data=phate_df_shuffled, x="PHATE1", y="PHATE2", hue="phase", palette="tab10", s=10, ax=axes[0, 1])
axes[0, 1].set_title("PHATE of Data by Labels")
axes[0, 1].legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3, markerscale=2)

sns.scatterplot(data=phate_df_shuffled, x="PHATE1", y="PHATE2", hue="Cd34", palette="viridis", s=10, ax=axes[1, 0])
axes[1, 0].set_title("PHATE of Data by Cd34")
axes[1, 0].legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3, markerscale=2)

sns.scatterplot(data=phate_df_shuffled, x="PHATE1", y="PHATE2", hue="Xcr1", palette="viridis", s=10, ax=axes[1, 1])
axes[1, 1].set_title("PHATE of Data by Xcr1")
axes[1, 1].legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3, markerscale=2)

sns.scatterplot(data=phate_df_shuffled, x="PHATE1", y="PHATE2", hue="Cd14", palette="viridis", s=10, ax=axes[2, 0])
axes[2, 0].set_title("PHATE of Data by Cd14")
axes[2, 0].legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3, markerscale=2)

sns.scatterplot(data=phate_df_shuffled, x="PHATE1", y="PHATE2", hue="Nkg7", palette="viridis", s=10, ax=axes[2, 1])
axes[2, 1].set_title("PHATE of Data by Nkg7")
axes[2, 1].legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3, markerscale=2)

sns.scatterplot(data=phate_df_shuffled, x="PHATE1", y="PHATE2", hue="N", palette="viridis", s=10, ax=axes[3, 0])
axes[3, 0].set_title("PHATE of Data by N")
axes[3, 0].legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3, markerscale=2)

sns.scatterplot(data=phate_df_shuffled, x="PHATE1", y="PHATE2", hue="S", palette="viridis", s=10, ax=axes[3, 1])
axes[3, 1].set_title("PHATE of Data by S")
axes[3, 1].legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3, markerscale=2)
...
# %%
sns.scatterplot(data=phate_df_shuffled, x="PHATE1", y="PHATE2", hue="pct_counts_scov", palette="viridis", s=10,)
plt.title("PHATE of Data by pct_counts_scov")
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3, markerscale=2)
plt.show()

# %%
sc.pl.embedding(adata, basis="X_phate_X_scVI_latent",
                color=["sample",
                       "condition",
                       "challenge",
                       "n_genes_by_counts",
                       ...
                       'Hba-a1', 'Hbq1b', 'Hba-a2', 'Hbq1a', 'Hbp1', 'Hbegf',#Hb genes], ncols=3, legend_fontsize=5)
                       ],
                       ncols=2,vmin="p0.05",vmax="p99.5",
                       frameon=False,show=False,layer="log_counts")
                       #cmap=seurat_cmap_adjusted
plt.savefig("adata_log_counts_PHATE_reduction_scvi_model_and_replicate.png", bbox_inches="tight",dpi=300)
plt.show()
# %%
...
sc.pl.umap(adata, color=["replicate","NKG7","CD4","CD8A","FLT3","FCER1A","CD14"], legend_fontsize=5)
...
umap_df_reshuffled = umap_df.sample(frac=1, random_state=2207).reset_index(drop=True)
# Plot UMAP
fig, axs = plt.subplots(4, 3, figsize=(20, 21))
plt.subplots_adjust(hspace=0.4, wspace=0.4)
...
for i, sample in enumerate(['pilot_mock', 'pilot_OTS206', 'pilot_wt']):
    row = (i // 3) + 1
    col = i % 3
    sns.scatterplot(x='UMAP1', y='UMAP2', facecolor='grey', alpha=0.3, data=umap_df_reshuffled, ax=axs[row, col], s=10)
...
with plt.rc_context():
    ...
    plt.subplots_adjust(hspace=0.4, wspace=0.4)
    sc.pl.umap(adata, color="sample", groups=["pilot_mock"], ax=axs[0], show=False, frameon=False)
    sc.pl.umap(adata, color="sample", groups=["pilot_OTS206"], ax=axs[1], show=False, frameon=False)    
    sc.pl.umap(adata, color="sample", groups=["pilot_wt"], ax=axs[2], show=False, frameon=False)    
plt.show()
...
# %%
sc.pl.umap(adata, color=["CD34","MKI67"], legend_fontsize=5,legend_loc="on data",show=False,frameon=False)

# %%
# Split by condition
with plt.rc_context():
    fig, axs = plt.subplots(4,1, figsize=(11, 24))
    sc.pl.umap(adata[adata.obs["sample"].isin(["pilot_mock"])],ax=axs[0],
               color="celltype_hint",frameon=False,show=False,title="LPS")
    sc.pl.umap(adata[adata.obs["sample"].isin(["pilot_OTS206"])],ax=axs[1],
                color="celltype_hint",frameon=False,show=False,title="OTS")
    sc.pl.umap(adata[adata.obs["sample"].isin(["pilot_wt"])],ax=axs[2],
               color="celltype_hint",frameon=False,show=False,title="WT")
    sc.pl.umap(adata,ax=axs[3],
...
with plt.rc_context():
    ...
    sc.pl.umap(adata[adata.obs["sample"].isin(["pilot_mock"])],ax=axs[0],
               color="celltype_hint",frameon=False,show=False,title="LPS",legend_loc="on data")
    sc.pl.umap(adata[adata.obs["sample"].isin(["pilot_OTS206"])],ax=axs[1],
               color="celltype_hint",frameon=False,show=False,title="OTS",legend_loc="on data")
    sc.pl.umap(adata[adata.obs["sample"].isin(["pilot_wt"])],ax=axs[2],
               color="celltype_hint",frameon=False,show=False,title="WT",legend_loc="on data")
    sc.pl.umap(adata,ax=axs[3],
...
with plt.rc_context():
    ...
    sc.pl.umap(adata[adata.obs["sample"] == "pilot_mock"],ax=axs[0,0],color="replicate",
               vmin="p01.15",vmax="p98.5",show=False,title="LPS")
    sc.pl.umap(adata[adata.obs["sample"] == "pilot_OTS206"],ax=axs[0,1],color="replicate",
                vmin="p01.15",vmax="p98.5",show=False,title="OTS")
    sc.pl.umap(adata[adata.obs["sample"] == "pilot_wt"],ax=axs[0,2],color="replicate",
                vmin="p01.15",vmax="p98.5",show=False,title="WT")
    sc.pl.umap(adata, color="phase", ax=axs[1,1], show=False, frameon=False,title="phase")
    sc.pl.umap(adata,ax=axs[1,2],color="replicate",show=False,title="merged")
    sc.pl.umap(adata, color="pct_counts_scov", ax=axs[1,0], show=False, frameon=False,title="SCOV counts")
...
with plt.rc_context():
    ...
    sc.pl.umap(adata, color="sample", groups=["pilot_mock"], ax=axs[0,0], show=False, frameon=False)
    sc.pl.umap(adata, color="sample", groups=["pilot_OTS206"], ax=axs[0,1], show=False, frameon=False)    
    sc.pl.umap(adata, color="sample", groups=["pilot_wt"], ax=axs[0,2], show=False, frameon=False)    
    sc.pl.umap(adata, color="phase", ax=axs[1,0], show=False, frameon=False)
    sc.pl.umap(adata, color="replicate", ax=axs[1,1], show=False, frameon=False)
    sc.pl.umap(adata, color="sample", ax=axs[1,2], show=False, frameon=False)

    plt.tight_layout()
    #plt.savefig("adata_harmony_model_replicate_PBMC_separated_samples_distribution.png")
    plt.show()
# %% Now use the predictions to plot the UMAP with the scVI reduction.
...
# Split by condition
with plt.rc_context():
    fig, axs = plt.subplots(4,1, figsize=(12, 21))
    sc.pl.umap(adata[adata.obs["sample"].isin(["pilot_mock"])],ax=axs[0],
               color="celltype_hint",frameon=False,show=False,title="LPS")
    ...
    sc.pl.umap(adata,ax=axs[3],
               color="celltype_hint",frameon=False,show=False,title="Merged")
    plt.savefig("umap_adata_constrained_annotation_split_LPS_OTS_WT_Merged.png")
    plt.show()

# %%
# Split by condition, legend on data.
with plt.rc_context():
    fig, axs = plt.subplots(1,4, figsize=(51, 12))
    sc.pl.umap(adata[adata.obs["sample"].isin(["pilot_mock"])],ax=axs[0],
               color="celltype_hint",vmin="p0.15",vmax="p98.5",frameon=False,show=False,legend_loc="on data",title="LPS")
    
    sc.pl.umap(adata[adata.obs["sample"].isin(["pilot_OTS206"])],ax=axs[1],
               color="celltype_hint",vmin="p0.15",vmax="p98.5",frameon=False,show=False,legend_loc="on data", title="OTS")
    
    sc.pl.umap(adata[adata.obs["sample"].isin(["pilot_wt"])],ax=axs[2],
               color="celltype_hint",vmin="p0.15",vmax="p98.5",frameon=False,show=False,legend_loc="on data", title="WT")
    
    sc.pl.umap(adata,ax=axs[3],
               color="celltype_hint",vmin="p0.15",vmax="p98.5",frameon=False,show=False,legend_loc="on data",title="Merged")
    plt.savefig("umap_adata_constrained_annotation_split_LPS_OTS_WT_Merged_legend_on.png")
    plt.show()
...
with plt.rc_context():
    ...
    sc.pl.umap(adata[adata.obs["replicate"] == "pilot_mock"],ax=axs[0,0],color="replicate",
               vmin="p01.15",vmax="p98.5",show=False,title="LPS")
    sc.pl.umap(adata[adata.obs["replicate"] == "pilot_OTS206"],ax=axs[0,1],color="replicate",
                vmin="p01.15",vmax="p98.5",show=False,title="OTS")
    sc.pl.umap(adata[adata.obs["replicate"] == "pilot_wt"],ax=axs[0,2],color="replicate",
                vmin="p01.15",vmax="p98.5",show=False,title="WT")
    sc.pl.umap(adata,ax=axs[1,0],color="pct_counts_scov",
                show=False,title="pct_counts_scov")
    sc.pl.umap(adata,ax=axs[1,1],color="phase",
                show=False,title="phase")
    sc.pl.umap(adata,ax=axs[1,2],color="replicate",
...
with plt.rc_context():
    ...
    sc.pl.umap(adata, color="replicate", groups=["pilot_mock"], ax=axs[0,0], show=False, frameon=False,title="LPS")
    sc.pl.umap(adata, color="replicate", groups=["pilot_OTS206"], ax=axs[0,1], show=False, frameon=False,title="OTS")    
    sc.pl.umap(adata, color="replicate", groups=["E_3"], ax=axs[0,2], show=False, frameon=False,title="WT")    
    
    sc.pl.umap(adata, color="pct_counts_scov", ax=axs[1,0], show=False, frameon=False,title="pct_counts_scov")
    sc.pl.umap(adata, color="phase", ax=axs[1,1], show=False, frameon=False,title="phase")
    sc.pl.umap(adata, color="sample", ax=axs[1,2], show=False, frameon=False,title="sample")
    plt.tight_layout()
...
# %%
adata.write_h5ad(r"C:\Users\emari\Documents\Environments\COVID_Etori_Demeter_hard_copy\CovidParty\data_analysis\Mock_OTS_WT\04_Load_and_scimilarity\adatas\adata_preprocessed_integrated_scVI_annotated_scimilarity.h5ad")
