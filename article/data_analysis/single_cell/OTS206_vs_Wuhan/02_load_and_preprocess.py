...
def pp(adata):
    ...
    adata.obs["mt_outlier"] = is_outlier(adata, "pct_counts_mt", lower_nmad=5, upper_nmad=5) | (adata.obs["pct_counts_mt"] > 5)#with 5 clusters are more clear
    #ribosomal genes
...
def mark_doublets_scrublet(adata,filter_doublets=False):
    ...
    if filter_doublets:
        ...
        adata_doublets = adata[adata.obs['doublet_info'] == 'doublet',:].copy()
        # save the doublets for each sample in a file for future reference
        adata_doublets.write_h5ad(r"C:\Users\emari\Documents\Environments\COVID_Etori_Demeter_hard_copy\CovidParty\data_analysis\Mock_OTS_WT\02_Load_and_preprocess\adatas\doublets\adata_doublets_"+adata.obs["sample"].iloc[0]+ ".h5ad")
...
for i,adata in enumerate(adatas_cleaned):
    adata.write_h5ad(r"C:\Users\emari\Documents\Environments\COVID_Etori_Demeter_hard_copy\CovidParty\data_analysis\Mock_OTS_WT\02_Load_and_preprocess\adatas\filtered\adata_filtered_"+adata.obs["sample"].iloc[0]+"_filtered_mt_5_hb_5_nmads_3_8_ribo_8_singlets_marked_cellcycle.h5ad")
