...
from chrysalis.utils import generate_random_colors, black_to_color, get_rgb_from_colormap, mip_colors, color_to_color, \
...
def plot_samples(adata: AnnData, rows: int, cols: int, dim: int, selected_comp: Union[int, str]='all',
                 ...
                 spot_size=2, rotation=0, wspace=0.5, hspace=0.5, title_col=None, **plot_kw):
    """
    ...
    for idx, i in enumerate(adata.obs[sample_col].cat.categories):
        ...
        if show_title:
            ...
            ax[idx].set_title(f'{obs_df[title_col][0]}')
...
def plot_weights(adata: AnnData, hexcodes: List[str]=None, seed: int=None, compartments: List[int]=None, ncols: int=4,
                 w: float=1.0, h: float=1.0, top_genes=20, title_size=15):
    """
    ...
    for idx, c in enumerate(expression_df.columns):
        ...
        ax[idx].set_title(f'Compartment {cnum}', fontsize=title_size)
    plt.tight_layout()